...
class   QFormsWidget_Date extends QFormsWidget {
    ...
    function    x_qDate($fecha, $cvt=null) {
        ...
        if(is_array($fecha)) {
            ...
            $fecha = '';
        /* }elseif( is_string($fecha) && preg_match('/^([0-9][0-9][0-9][0-9])[^0-9]([0-9]{1,2})[^0-9]([0-9][0-9])[^0-9]([0-9][0-9])[^0-9]([0-9][0-9])[^0-9]?([0-9][0-9])?/',$fecha,$r)) { // formato YYYY-MM-DD HH:MM:SS-n Postgress...
            $fecha = adodb_mktime($r[4],$r[5],$r[6],$r[2],$r[3],$r[1]); */
        }elseif( is_string($fecha) && preg_match('/^([0-9][0-9][0-9][0-9])[^0-9]([0-9]{1,2})[^0-9]([0-9]{1,2})$/',$fecha,$r)) { // formato YYYY-MM-DD o YYYY/MM/DD
            $fecha = adodb_mktime(0,0,0,$r[2],$r[3],$r[1]);
        }elseif( is_string($fecha) && preg_match('/^(2[0-9][0-9][0-9])([0-9][0-9])([0-9][0-9])$/',$fecha,$r)) { // formato YYYYMMDD
            $fecha = adodb_mktime(0,0,0,$r[2],$r[3],$r[1]);
        }elseif( is_string($fecha) && preg_match('/^([0-9]{1,2})[^0-9]([0-9]{1,2})[^0-9]([0-9]{1,4})$/',$fecha,$r)) { // DD/MM/YYYY
            $fecha = adodb_mktime(0,0,0,$r[2],$r[1],$r[3]);
        }elseif( is_string($fecha) && preg_match('/^([0-9][0-9])[^0-9]([0-9]{1,2})[^0-9]([0-9][0-9][0-9][0-9])[^0-9]([0-9][0-9])[^0-9]([0-9][0-9])$/',$fecha,$r)) { // formato DD-MM-YYYY HH:MM o DD/MM/YYYY HH:MM
